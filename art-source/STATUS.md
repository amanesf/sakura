...
- **花畑の小花**（`app/src/scene/flowers.ts`）: 春の菜の花・タンポポが依然
  Icosahedronの単色ドット（procedural）のまま。
- **散る花びら/落ち葉**（`app/src/scene/sheddingParticles.ts`）: 依然単色の
  小さい平面パーティクル（procedural）のまま。
- 上記2つも同じ「クロップ→Gemini生成→chromakey抽出」パターンを適用できる
  想定（下草と同様、小さいスプライト画像に置き換える形）。
- 空（`sky.ts`）は手続きシェーダーのまま（グラデーション自体は簡易な実装だが、
  1.5章の対象に明記されている）。他と違って「静止画テクスチャに置き換える」
  のではなく雲・光条などの動的表現との両立が必要なため、着手方法を要検討。
